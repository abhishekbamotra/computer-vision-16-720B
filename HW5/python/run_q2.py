...
for itr in range(max_iters):
    ...
    for xb,yb in batches:
       # pass
        ...
        pred_out = forward(frwd1,params, 'output', softmax)

        loss, acc = compute_loss_and_acc(yb, pred_out)
        
        total_loss +=  loss
        avg_acc += acc
        
        del_val = backwards(pred_out - yb,params, 'output', linear_deriv)
...
x_val, y_val = get_random_batches(x,y,5)[0]

frwd1 = forward(x_val,params,'layer1')
pred_val = forward(frwd1,params,'output',softmax)

del_val = backwards(pred_val - y_val,params,'output',linear_deriv)
backwards(del_val,params,'layer1',sigmoid_deriv)
...
for k,v in params.items():
    ...
    ##### your code here #####
    ##########################
    if k in ['Wlayer1','Woutput']: 
        for x in range(v.shape[0]):
            for y in range(v.shape[1]):
                param_copy1 = copy.deepcopy(params_orig)
                param_copy1[k][x][y] -= eps
                
                param_copy2 = copy.deepcopy(params_orig)
                param_copy2[k][x][y] += eps

                frwd1 = forward(x_val,param_copy1,'layer1')
                pred1_val = forward(frwd1,param_copy1,'output',softmax)
                loss1, no_need = compute_loss_and_acc(y_val, pred1_val)

                frwd2 = forward(x_val,param_copy2,'layer1')
                pred2_val = forward(frwd2,param_copy2,'output',softmax)
                loss2, no_need = compute_loss_and_acc(y_val, pred2_val)
                
                total_save = (loss2 - loss1) / (2*eps) 
                params['grad_' + k][x][y] = total_save

    if k in ['blayer1','boutput']:
        for el in range(v.size):
            param_copy = copy.deepcopy(params_orig)
            param_copy[k][el] -= eps
            
            param_copy2 = copy.deepcopy(params_orig)
            param_copy2[k][el] += eps
            
            frwd1 = forward(x_val, param_copy, 'layer1')
            pred1_val = forward(frwd1, param_copy, 'output', softmax)
            loss1, no_need = compute_loss_and_acc(y_val, pred1_val)
